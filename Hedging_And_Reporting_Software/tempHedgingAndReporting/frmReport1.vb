...
Public Class frmReport1
    ...
    Private Sub frmReport1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dateTimePickerOrderDate.CustomFormat = "dd/MM/yy"
    End Sub

    Private Sub btnExportReport1_Click(sender As Object, e As EventArgs) Handles btnExportReport1.Click

        Dim onlyDate = Format(CDate(dateTimePickerOrderDate.Value.ToString), "dd-MMM-yy")
        onlyDate = onlyDate.ToUpper()
        ...
        sqlCommand = "SELECT DISTINCT CONCAT(stem_products_main.stem_company ||'/'|| stem_products_main.stem_series || '/' || stem_products_main.stem_number, stem_products_main.PCODE) AS CONCATENATE,
                    ...
                    AND stems_main.stem_number=stem_products_main.stem_number where stems_main.DELIVERY_DATE like '%" & onlyDate & "%' AND (STEMS_MAIN.STEM_COMPANY = '003' OR STEMS_MAIN.STEM_COMPANY = '009')"
